...
public class BookController {
	...
	public ResponseEntity<Optional<Book>> getBookById(@PathVariable int id){
		...
		if(bk != null) {
			return new ResponseEntity<Optional<Book>>(bk,HttpStatus.OK);
		}else {
			return new ResponseEntity<Optional<Book>>(bk, HttpStatus.NO_CONTENT);
		}
	}
	...
	public ResponseEntity<Book> updateBookById(@PathVariable int id, @RequestBody Book book){
		...
		if(bk != null) {
			...
			return new ResponseEntity<Book>(bk, HttpStatus.BAD_REQUEST);
		}
	}
